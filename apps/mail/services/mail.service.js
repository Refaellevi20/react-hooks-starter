...
function query(filterBy = {}) {
    return storageService.query(MAIL_KEY)
        .then(mails => {
            ...
            //* chacking if isStared true or false
          ...
          //^ means This means that only emails that are neither in the trash nor in draft status will remain in the mails array. 
            if (filterBy.status === 'all') {
                mails = mails.filter(mail => mail.status !== 'trash' && mail.status !== 'draft')
